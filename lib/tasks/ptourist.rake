namespace :ptourist do
  ...
  def create_inquiry organizer, inq
    puts "building inquiry for #{inq[:question]}, by #{organizer.name}"
    inquiry=Inquiry.create(:creator_id=>organizer.id,:question=>inq[:question])
    organizer.add_role(Role::ORGANIZER, inquiry).save
  end
  ...
  desc "deletes things, images, links and inquiries"
  ...
  desc "reset things, images, links and inquiries"
  task subjects: [:users] do
    puts "creating things, images, links and inquiries"
...
"}
    ...
    organizer=get_user("alice")
    inquiry={:question=>"How many hours do people typically spend here?"}
    create_inquiry organizer, inquiry

    organizer=get_user("greg")
    inquiry={:question=>"Is it kid-friendly?"}
    create_inquiry organizer, inquiry

    organizer=get_user("alice")
    inquiry={:question=>"What are the opening hours?"}
    create_inquiry organizer, inquiry
  end
